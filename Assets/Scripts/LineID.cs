...
public class LineID : MonoBehaviour
{
    ...
    public void RePlacement()
    {
        ...
        /*  if ((secondObject.transform.position.x - firstObject.transform.position.x > 0 && secondObject.transform.position.y - firstObject.transform.position.y > 0) || (secondObject.transform.position.x - firstObject.transform.position.x < 0 && secondObject.transform.position.y - firstObject.transform.position.y < 0))
              this.transform.rotate = new Vector3(0, Mathf.Atan((secondObject.transform.position.x - firstObject.transform.position.x) / (secondObject.transform.position.y - firstObject.transform.position.y)), 0);
          else
              this.transform.rotate = new Vector3(0, Mathf.Atan((secondObject.transform.position.x - firstObject.transform.position.x) / (secondObject.transform.position.y - firstObject.transform.position.y)) + 180, 0);
      */
    }
}
